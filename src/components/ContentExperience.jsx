...
const ContentExperience = ({ experienciaDB }) => {
	return (
				...
				{experienciaDB.map((item) => (
					...
					>
						<div className="border-[.2rem] p-4 text-white rounded-md border-primary-light shadow-4xl shadow-primary">
							...
							<h3 className="mt-2 font-bold text-xl">{item.company}</h3>
							<p className="mb-2 -mt-1 text-primary-light">{item.place}</p>
							<p className="font-bold text-xl pb-1 mb-3 border-b-2">
								{item.title}
							</p>
							<p>
								{item.content.split("\n").map((line, index) => (
